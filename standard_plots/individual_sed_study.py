...
def prepare_data(hdf5_data, sfh, phot_data, ids_sed, index, nbands):
    ...
    print 'max SFR and mstar', max(gal_props[:,3]), max(gal_props[:,1])
...
def main(model_dir, outdir, redshift_table, subvols, obsdir):
    ...
    for index, snapshot in enumerate(snapshots):
        ...
        (SEDs_dust, SEDs_nodust, total_sfh, sb_sfh, disk_sfh, gal_props) = prepare_data(hdf5_data, sfh, seds, ids, index, nbands)
