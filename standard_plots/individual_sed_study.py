...
def plot_fluxdisk(plt, outdir, obsdir, h0, fdisk_emission, fdisk_emission_tot):
    ...
    ytit="$\\rm f_{\\rm disk}/ f_{\\rm total}$"

    xmin, xmax, ymin, ymax = 3.0, 7.0, 0, 1
    ...
    indices  = (0, 1, 2, 3, 4, 5) 
    colors = ('k','purple','Navy','Green','OrangeRed','red')
    xplot = np.log10(lambda_bands) 

    for cols, idx in zip(indices):

        ax.plot(xplot,fdisk_emission_tot[idx,:],cols)

    common.savefig(outdir, fig, "Flux_Fraction_Reshift_Evolution.pdf")
...
def plot_individual_seds_z2(plt, outdir, obsdir, h0, SEDs_dust_z2, total_sfh_z2, disk_sfh_z2, sb_sfh_z2, gal_props_z2, LBT_z2):
    ...
    indices  = (0, 1) #, 6, 7, 8)
    sfrs     = (500.0, 10.0)
    labelsfrs=  ('$\\rm SFR/M_{\odot} yr^{-1}= 500$', '$\\rm SFR/M_{\odot} yr^{-1}= 10$')
    ...
    common.savefig(outdir, fig, "SEDs_SFRs_z2_twopanel.pdf")
...
def plot_individual_seds(plt, outdir, obsdir, h0, SEDs_dust_z0, SEDs_nodust_z0, total_sfh_z0, gal_props_z0, LBT):
    ...
    colors = ('Indigo','purple','Navy','Green','OrangeRed','red','DarkRed','FireBrick','Crimson','IndianRed','LightCoral','Maroon','brown','Sienna','SaddleBrown','Chocolate','Peru','DarkGoldenrod','Goldenrod','SandyBrown')
    ...
    for subplot, idx in zip(subplots, indices):
        ...
        if(len(gal_props_z0[ind,0]) > 0):
           ...
           for xi,yi,yind,c in zip(xplot,yplot,yplotnd,colors):
               ax.plot(xi,yi, 'x', markersize=6, color=c)
               ax.plot(xi,yind, 'x', markersize=3, color=c)
    ...
    for subplot, idx in zip(subplots, indices):
        ...
        if(len(gal_props_z0[ind]) > 0):
           ...
           for gal in range(0,numgals): 
               ...
               lowsfr = np.where(sfh_in < 0.01)
               sfh_in[lowsfr] = 0.001
               print 'galaxy type', typesg[0,gal]
               if(typesg[0,gal] == 0):
...
def prepare_data(hdf5_data, sfh, phot_data, ids_sed, index, nbands, fdisk_emission, fdisk_emission_tot):
    ...
    for m in range(0,mbins):
        ind = np.where((np.log10(gal_props[:,1]) > xmf[m]-dm/2.0) & (np.log10(gal_props[:,1]) <= xmf[m]+dm/2.0))
        ngals  = len(gal_props[ind])
        if(ngals > 9):
           SEDsin = SEDs_dust[ind,:]
           for b in range(0,nbands):
               fdisk_emission[index,b,m] = np.sum(pow(10.0,SEDsin[:,3,b]/(-2.5)))/np.sum(pow(10.0,SEDsin[:,4,b]/(-2.5)))
               #calculate total flux break-down only once
               if(m == 0):
                  fdisk_emission_tot[index,b] = np.sum(pow(10.0,SEDs_dust[:,3,b]/(-2.5)))/np.sum(pow(10.0,SEDs_dust[:,4,b]/(-2.5)))
...
def main(model_dir, outdir, redshift_table, subvols, obsdir):
    ...
    for index, snapshot in enumerate(snapshots):
        ...
        if(index == 0):
              ...
              fdisk_emission_tot = np.zeros(shape = (len(z), nbands))

        (SEDs_dust, SEDs_nodust, total_sfh, sb_sfh, disk_sfh, gal_props) = prepare_data(hdf5_data, sfh, seds, ids, index, nbands, fdisk_emission, fdisk_emission_tot)
        ...
        if(index == 0):
            ...
            LBT_z0 = LBT
            plot_individual_seds(plt, outdir, obsdir, h0, SEDs_dust_z0, SEDs_nodust, total_sfh_z0, gal_props_z0, LBT_z0)
