...
def bressan_model(sfr,seds_nodust_tot):
    ...
    def synchrotron_lum(SFR, nu):
        ...
        cc_chabrier = 0.011148
        LNT = 6.1*10**(-2)*(1.49/0.408)**(-0.8) ##2.16 x10^(-2) W/Hz/10^(21)
        ENT = LNT/cc_chabrier ##1.31 x 10^(23) W/Hz/10^(21)
...
def prepare_data(hdf5_data, ext_data, seds, seds_lir, index, zsnap, obsdir, sm_irx, seds_nodust):
    ...
    SEDs_nodust_total = seds_nodust[2] #total absolute magnitudes without dust
    ...
    lir_disk = lir_disk[0,:]
    mag_ion = SEDs_nodust_total[1,:]
    print('mag_ion.shape', mag_ion.shape)
    ...
    pos = np.where((SEDs_dust_total[4,:] > -30) & (SEDs_dust_total[4,:] < -10) & (mstars > 1e7) & (zgas > 0) & (typeg == 0))
    mag_ion_total = mag_ion[pos]
    ...
    lir_total_in= lir_total[pos]
    irx = lir_total_in / lfuv_total
    ...
    sfr = sfr[pos]
    lir_total_in = lir_total_in[pos]
    sm_irx[index,:] = us.wmedians(x=mstar_in, y=irx, xbins=xmf)
    
    ###Compute Radio Emission with Bressan model###
    
    rad_lum = bressan_model(sfr/1e9,mag_ion[pos])

    qir = np.log10(lir_total_in * Lsun /3.75e12) - np.log10(rad_lum[0])
    
    
    ind = np.where((mstar_in > 9) & (ssfr_in > -0.5) & (qir > -10) & (qir < 10))
    print("This is the median qir", np.median(qir[ind]))
...
def plot_irx_beta(plt, outdir, obsdir, irx, beta, mstar, zgas_in, ssfr, mdust, eta, tau_birth, tau_screen, z, dust_model,qir):
    ...
    def plot_obs(ax):
        ...
        ax.plot(xv, yv, linestyle='dashdot', color='k', label='Reddy+15')
    ...
    def plot_properties_in_irx_beta(prop, name, label, binsx=30, binsy=30, vmin=-1.5, vmax=0.5):
       ...
       for i,subplot in enumerate(subplots):
           ...
           meds_errors[i] = meds
           im = ax.hexbin(x, y, prop[inp], xscale='linear', yscale='linear', gridsize=(binsx,binsy), vmin=vmin, vmax=vmax, cmap='magma', mincnt=20, reduce_C_function=np.median)
    ...
    meds_errors = plot_properties_in_irx_beta(qir, 'qir', 'qir', binsx=30, binsy=30, vmin=1.5, vmax=3)
...
def main(modeldir, outdir, redshift_table, subvols, obsdir):
    ...
    for index, snapshot in enumerate(redshift_table[zlist]):
        ...
        (volh, h0, irx, beta, mstar_in, zgas_in, ssfr, mdust, 
         eta, tau_birth, tau_screen,qir) = prepare_data(hdf5_data, ext_data, seds, seds_lir, index, zlist[index], obsdir, sm_irx, seds_nodust)
