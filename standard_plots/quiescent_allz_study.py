...
def prepare_data(hdf5_data, index, obsdir, nbrightgals, nbrightgals_err, csfr):
    ...
    ind = np.where(mstar_tot >= sm_thresh2)
    ...
    ind = np.where((mstar_tot >= sm_thresh2) & (sfr_tot/mstar_tot < ssfr_thresh)) 
    brightgals = mstar_tot[ind]
    ...
    ind = np.where((mstar_tot >= sm_thresh2) & (sfr_tot/mstar_tot < ssfr_thresh2)) 
    brightgals = mstar_tot[ind]
...
def main(model_dir, outdir, redshift_table, subvols, obsdir):
    ...
    for index, snapshot in enumerate(redshift_table[zlist]):
        ...
        (volh, h0) = prepare_data(hdf5_data, index, obsdir, nbrightgals, nbrightgals_err, csfr)
    ...
    if(Write_Tables == True): 
       ...
       print('#N1: number density of galaxies with Mstar>1e10Msun:')
       print('#N2: number density of galaxies with Mstar>3.9e10Msun')
       print('#N3: number density of galaxies with Mstar>3.9e10Msun & SSFR<1e-10yr^-1')
       print('#N4: number density of galaxies with Mstar>1e10Msun & SSFR<1e-10yr^-1')
       print('#N5: number density of galaxies with Mstar>3.9e10Msun & SSFR<1e-11yr^-1')
       print('#N6: number density of galaxies with Mstar>1e10Msun & SSFR<1e-11yr^-1')
       ...
       print('# ')
       print('#columns:')
       print('#redshift N1 N2 N3 N4 N5 N6')
       for a,b,c,d,e,f,g in zip(zlist,ngals[3,j,:],ngals[5,j,:],ngals[6,j,:],ngals[7,j,:],ngals[9,j,:],ngals[10,j,:]):
           print(a,b,c,d,e,f,g)
       print('# ')
       print('#redshift N1(WE) N2(WE) N3(WE) N4(WE) N5(WE) N6(WE)')
