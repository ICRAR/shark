...
DataSet DataSet::create(shark::hdf5::AbstractGroup &parent, const std::string &name, const DataType& dataType,
    ...
    return DataSet(name, H5Dcreate(parent.getHandle(), name.c_str(), dataType.getHandle(), dataSpace.getHandle(), H5P_DEFAULT,
...
herr_t DataSet::write(const void *buf, const DataType &memDataType, const DataSpace &memSpace, const DataSpace &fileSpace) {
    return H5Dwrite(getHandle(), memDataType.getHandle(), memSpace.getHandle(), fileSpace.getHandle(), H5P_DEFAULT, buf);
}
...
void DataSpace::selectHyperslab(const HyperslabSelection& op, const std::vector<hsize_t>& start, const std::vector<hsize_t>& stride, const std::vector<hsize_t>& count, const std::vector<hsize_t>& block) {
    // Assert all same length & same as rank
    H5Sselect_hyperslab(getHandle(), static_cast<H5S_seloper_t>(op), start.data(), stride.data(), count.data(), block.data());
}
